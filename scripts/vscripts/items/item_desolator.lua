...
function item_imba_desolator:OnSpellStart()
	if IsServer() then
		...
		local desolator_projectile = {
			Ability				= self,
			EffectName			= "particles/item/desolator_active_c.vpcf",
...
function item_imba_desolator:OnProjectileHit(target, target_loc)
	if IsServer() and target then
		...
		-- Play hit sound
		target:EmitSound("ItemDesolatorHit")
		...
		local effect_pfx = ParticleManager:CreateParticle("particles/tower_bad_face_end_sparks.vpcf", PATTACH_CUSTOMORIGIN, enemy)
...
function item_imba_desolator_2:OnSpellStart()
	...
	local desolator_projectile = {
		Ability				= self,
		EffectName			= "particles/item/desolator2_active.vpcf",
